...
local username_output="%(!..$yellow$username_command$green@)"
local hostname_output="$cyan$hostname_command$reset"
local current_dir_output="$magenta$current_dir$reset"
...
GIT_PROMPT='$(out=$(git_prompt_info)$(git_prompt_status)$(git_remote_status);if [[ -n $out ]]; then printf %s " $white($white$out$white)$reset";fi)'
PROMPT+="$GIT_PROMPT"
PROMPT+=" $last_command_output$white\$$reset "
RPROMPT=''
